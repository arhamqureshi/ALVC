import time, threading, ctypes, os, json, requests
from tkinter import (
...
class GUI(Frame):
    ...
    def __init__(self, gui):
        ...
        self.version = Label(self.information, text=self.__version__)
        self.version.configure(font="Helvetica 9")
        self.version.grid(row=1, column=0)

        self.check_latest()

        self.link = Label(self.information, text="Settings Documentation", fg="blue", cursor="hand2", font="Helvetica 10 bold")
        ...
        self.link.bind("<Button-1>", lambda e: webbrowser.open_new("https://github.com/arhamqureshi/ALVC/blob/main/README.md#how-it-works"))
        ...
        self.knocks_elims_found_label = Label(self.progress, text="Potential Knocks/Elims Found: 0")
        self.knocks_elims_found_label.grid(row=5, column=0, padx=10, pady=5, sticky="w")
        ...
        self.created_by_link = Label(self.credits, text="Created By Arham Qureshi", fg="blue", cursor="hand2")
        self.created_by_link.grid(row=0, column=0)
        self.f = font.Font(self.created_by_link, self.created_by_link.cget("font"))
        self.f.configure(underline=True, family="Helvetica", size="8")
        self.created_by_link.configure(font=self.f)
        self.created_by_link.bind("<Button-1>", lambda e: webbrowser.open_new("https://github.com/arhamqureshi/"))

        self.report_bug = Label(self.credits, text="Report a Bug", fg="blue", cursor="hand2")
        self.report_bug.grid(row=0, column=1)
        ...
        self.support_me_link = Label(self.credits, text="Support Me!", fg="blue", cursor="hand2")
        self.support_me_link.grid(row=0, column=2)
        self.f = font.Font(self.support_me_link, self.support_me_link.cget("font"))
        self.f.configure(underline=True, family="Helvetica", size="8")
        self.support_me_link.configure(font=self.f)
        self.support_me_link.bind("<Button-1>", lambda e: webbrowser.open_new("https://www.paypal.com/cgi-bin/webscr?cmd=_donations&business=WALNKNVGR4L8S&currency_code=AUD"))
    ...
    def check_latest(self):
        """
        Check the latest release of the program
        """
        response = requests.get("https://api.github.com/repos/arhamqureshi/ALVC/releases/latest")
        response = response.json()
        if "name" in response:
            latest = response["name"]
            latest = latest.split("v")[1]
            current = self.__version__.split("v")[1]
            if current < latest:
                self.version_status = Label(self.information, text="v{} Update Available".format(latest), fg="blue", cursor="hand2")
                self.version_status.grid(row=1)
                self.f = font.Font(self.version_status, self.version_status.cget("font"))
                self.f.configure(underline=True, family="Helvetica", size="9")
                self.version_status.configure(font=self.f)
                self.version_status.bind("<Button-1>", lambda e: webbrowser.open_new("https://github.com/arhamqureshi/ALVC/releases/latest"))
            else:
                self.version_status = Label(self.information, text="{} (latest)".format(self.__version__))
                self.version_status.configure(font="Helvetica 9")
                self.version_status.grid(row=1)

    def welcome_message(self):
        settings = self.get_settings_data()
        if not settings['read_welcome_message']:
            messagebox.showinfo("Welcome!", "Welcome to Apex Legends Video Clipper (ALVC)! \nPlease read the settings documentation before using the program, enjoy :)")
    ...
    def create_settings_file(self):
        """
        Creates a settings file with the default settings if it doesn't exist already.
        """

        if ".alvc-settings.json" not in os.listdir("."):
            default_settings = {
                ...
                "input_directory": "",
                "output_directory": "",
                "read_welcome_message": False
            }

            with open(".alvc-settings.json", "w") as write_file:
    ...
    def get_settings_data(self):
        with open(".alvc-settings.json", "r") as read_file:
    ...
    def stop_analysis(self, analysis_complete=False):
        ...
        if analysis_complete:
            messagebox.showinfo("Analysis Complete", "{} Clip/s exported to {}".format(self.knocks_elims_found, self.output_directory.get()))
        else:
            self.analyse_gameplay.raise_exception()
            messagebox.showinfo("Partial Analysis", "{} Clip/s captured before cancellation exported to {}".format(self.knocks_elims_found, self.output_directory.get()))
        
        self.knocks_elims_found = 0
        self.update_progress("knocks_elims", 0, 0, 0)
...
class AnalyseGameplay(threading.Thread): 
    ...
    def raise_exception(self):
        """
        This is used to raise an exception which cancels the process.
        """
